...
// GET /api/invoices  — liste toutes les factures de l'utilisateur
...
// POST /api/invoices  — enregistrer une facture pour une réservation
router.post('/', async (req, res) => {
  try {
    ...
    // Générer un numéro de facture unique par utilisateur
    const { rows: countRows } = await pool.query('SELECT COUNT(*)::int AS n FROM invoices WHERE user_id = $1', [req.user.id]);
    const next = countRows[0].n + 1;
